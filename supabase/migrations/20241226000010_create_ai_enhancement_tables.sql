-- =============================================
-- AI ENHANCEMENT TABLES - MAXPULSE APP-CENTRIC APPROACH
-- Migration: 20241226000006_create_ai_enhancement_tables.sql
-- Purpose: Create tables for intelligent product recommendations and wellness knowledge
-- Note: MAXPULSE App tables excluded (built separately)
-- =============================================
...
CREATE TABLE wellness_knowledge_base (
  ...
  layman_explanation TEXT NOT NULL, -- Simple explanation for users
  scientific_backing TEXT NOT NULL, -- Research-based explanation
  research_citations JSONB DEFAULT '[]', -- Array of study references
  
  -- Frank Assessment Messages
  objective_assessment_template TEXT, -- "Your X score indicates..."
  frank_recommendation_template TEXT, -- "You need to..."
  expected_timeline TEXT, -- "Results in 2-3 weeks"
  
  -- Product Recommendations
  recommended_products JSONB DEFAULT '[]', -- Array of product IDs
  contraindicated_products JSONB DEFAULT '[]', -- Products to avoid
  maxpulse_app_features JSONB DEFAULT '[]', -- Relevant app features
  ...
  -- Metadata
  created_by UUID, -- Will reference admin when admin tables exist
...
CREATE INDEX idx_wellness_knowledge_category ON wellness_knowledge_base(category);
CREATE INDEX idx_wellness_knowledge_severity ON wellness_knowledge_base(severity_level);
CREATE INDEX idx_wellness_knowledge_active ON wellness_knowledge_base(is_active);
...
CREATE TABLE product_bundles (
  ...
  -- Bundle Contents
  includes_maxpulse_app BOOLEAN DEFAULT true, -- Almost all bundles include the app
  maxpulse_app_duration_months INTEGER DEFAULT 3, -- App subscription length
  physical_products JSONB NOT NULL DEFAULT '[]', -- Array of {product_id, quantity}
  digital_products JSONB DEFAULT '[]', -- Additional digital products
  ...
  min_score_threshold DECIMAL(5,2) DEFAULT 0.0, -- Minimum assessment score to recommend
  ...
  app_configuration JSONB DEFAULT '{}', -- Specific app settings for this bundle
  /*
  Example app_configuration:
  {
    "hydration_goal_liters": 2.5,
    "sleep_target_hours": 7.5,
    "daily_step_goal": 10000,
    "primary_focus": ["sleep", "hydration"],
    "reminder_frequency": 60
  }
  */
  
  -- Messaging
  value_proposition TEXT, -- Why this bundle is perfect
  expected_results TEXT, -- What user can expect
  scientific_backing TEXT, -- Research supporting this combination
...
CREATE INDEX idx_product_bundles_priority ON product_bundles(recommendation_priority);
...
CREATE TABLE recommendation_rules (
  ...
  -- Trigger Conditions (JSON-based branching logic)
  ...
  Example trigger_conditions:
  {
    ...
    "responses": {
      ...
      "response_values": ["less_than_6", "4_to_5_hours"]
    },
    "priority_selection": ["health", "both"]
  }
  */
  ...
  {
    "primary_products": ["prod_magnesium_001", "prod_sleep_tracker_002"],
    "recommended_bundles": ["bundle_sleep_optimization"],
    "maxpulse_app_features": ["sleep_tracking", "bedtime_reminders"],
    "urgency_level": "high",
    "confidence_score": 0.92,
    "personalized_message": "Your sleep patterns indicate significant optimization opportunities",
    "upsell_opportunity": {
      "bundle_id": "bundle_complete_wellness",
      "trigger_timing": "after_2_weeks",
      "message": "Ready to optimize your entire lifestyle?"
    }
  }
  */
  
  -- Scientific Messaging
  layman_explanation TEXT, -- Simple explanation for this rule
  scientific_justification TEXT, -- Research backing this recommendation
  frank_assessment TEXT, -- Objective truth about user's situation
...
CREATE TABLE ai_recommendation_results (
  ...
  individual_responses JSONB DEFAULT '{}', -- Key question responses
  ...
  layman_explanations JSONB DEFAULT '{}', -- Per-category explanations
  scientific_backing JSONB DEFAULT '{}', -- Research citations per category
  frank_assessments JSONB DEFAULT '{}', -- Objective assessments per category
  
  -- MAXPULSE App Recommendations
  app_configuration JSONB DEFAULT '{}', -- Personalized app settings
  /*
  Example app_configuration:
  {
    "hydration_goal_liters": 2.5,
    "hydration_reminder_frequency": 60,
    "sleep_target_hours": 7.5,
    "sleep_bedtime": "22:30",
    "daily_step_goal": 12000,
    "mood_tracking_frequency": "daily",
    "primary_focus_areas": ["sleep", "hydration"],
    "lifestyle_constraints": ["busy_schedule", "travel_frequent"]
  }
  */
  ...
  expected_outcomes JSONB DEFAULT '{}', -- What user can expect and when
...
-- Add indexes for performance
CREATE INDEX idx_ai_recommendation_session ON ai_recommendation_results(session_id);
CREATE INDEX idx_ai_recommendation_assessment ON ai_recommendation_results(assessment_results_id);
CREATE INDEX idx_ai_recommendation_urgency ON ai_recommendation_results(urgency_level);
CREATE INDEX idx_ai_recommendation_confidence ON ai_recommendation_results(confidence_score);

-- =============================================
-- LIFESTYLE MESSAGING TEMPLATES - Science-backed messaging
-- =============================================
CREATE TABLE lifestyle_messaging_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  -- Science-Backed Content
  layman_explanation TEXT NOT NULL, -- "Your cells are like tiny factories..."
  scientific_backing TEXT NOT NULL, -- "Research shows cellular dehydration..."
  research_citations JSONB DEFAULT '[]', -- Array of study references
  
  -- Frank Messaging
  objective_assessment TEXT NOT NULL, -- "You're dehydrated 70% of the time"
  frank_recommendation TEXT NOT NULL, -- "You need to drink 2.5L daily, not 1.2L"
  expected_timeline TEXT, -- "Results in 2-3 weeks"
  
  -- MAXPULSE App Integration
  app_feature_highlight TEXT, -- How the app solves this specific issue
  app_benefit_message TEXT, -- "The MAXPULSE App will track this precisely"
  
  -- Demographic Targeting
  target_demographics JSONB DEFAULT '{}', -- Age, gender, lifestyle targeting
  
  -- Metadata
  created_by UUID, -- Will reference admin when admin tables exist
  is_active BOOLEAN DEFAULT true,
  usage_count INTEGER DEFAULT 0, -- Track which messages are most effective
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add indexes for performance
CREATE INDEX idx_messaging_templates_type ON lifestyle_messaging_templates(message_type);
CREATE INDEX idx_messaging_templates_category ON lifestyle_messaging_templates(health_category);
CREATE INDEX idx_messaging_templates_severity ON lifestyle_messaging_templates(severity_level);
CREATE INDEX idx_messaging_templates_active ON lifestyle_messaging_templates(is_active);

-- =============================================
-- BUNDLE PERFORMANCE TRACKING - Analytics for optimization
-- =============================================
CREATE TABLE bundle_performance_tracking (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  conversion_time_minutes INTEGER, -- Time from recommendation to purchase
  
  -- A/B Testing Support
  variant_id TEXT, -- For testing different bundle presentations
  test_group TEXT, -- Control, variant_a, variant_b, etc.
  
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add indexes for performance
CREATE INDEX idx_bundle_performance_bundle ON bundle_performance_tracking(bundle_id);
CREATE INDEX idx_bundle_performance_session ON bundle_performance_tracking(session_id);
CREATE INDEX idx_bundle_performance_purchased ON bundle_performance_tracking(was_purchased);
CREATE INDEX idx_bundle_performance_date ON bundle_performance_tracking(recommended_at);

-- =============================================
-- SEED DATA - Initial wellness knowledge and bundles
-- =============================================
